<div class="register-modal" id="registerModal" aria-hidden="true">
    <div class="register-dialog" role="dialog" aria-modal="true" aria-labelledby="registerTitle">
        ...
        <form class="register-card" id="publicRegisterForm" method="POST" action="{{ route('register.basic') }}">
            @csrf
            ...
            <section class="register-step" data-register-step="form" hidden>
                ...
                <div class="register-otp-box otp-disabled">
                    <span>OTP Nonaktif Sementara</span>
                    <p data-otp-text>OTP belum digunakan. Nanti akan disambungkan melalui pihak ketiga resmi.</p>
                    <button type="button" class="secondary-btn" disabled>Menunggu Integrasi</button>
                </div>
                <p class="login-error" data-register-error @if (! $errors->any()) hidden @endif>{{ $errors->first() }}</p>
